...
**Squash SHA.** `TBD` (PR #TBD). Resolution at Phase-6 Task 6.9 sweep per established "absorb into next sweep" cross-phase convention.
...
| 15 | Manual-dispatch parity gate | green (paths-filter quirk applies; `qualia-shell/playwright.baseline.config.ts` at qualia-shell/-root NOT covered by `qualia-shell/src/**` paths-filter) | **TBD** — `gh workflow run "AppFolio Parity Gate" -R NovaTrustSolutions/dwellium-per-spec --ref phase-6/task-6.8-feature-spec-ci-integration` at PR open; mirrors 5.3/5.4/5.5/5.6/5.7/6.5/6.6/6.7 precedent (10-task cross-phase scope) | §6 |
| 16 | CodeRabbit review | pass | **TBD** (PR status-check post-open) | §6 |
...
- PR title: `feat(phase-6): Task 6.8 — feature spec CI integration (#TBD)`
...
- Manual-dispatch fallback: **TBD** — executed at PR open per established paths-filter quirk precedent.
- CodeRabbit review: **TBD** (PR status-check post-open).
- Squash-merge target: `main`.
- Post-merge sweep ✅ **TO BE EXECUTED AT 6.9 SWEEP** per absorb-into-next-sweep precedent (will be **12 consecutive sweep-resolutions** cross-phase: meta-PR #44 → 6.1a → 6.1b → 6.1c → 6.2 → 6.3 → 6.4 → 6.5 → 6.6 → 6.7 → 6.8 → **6.9**): 6.8 squash SHA → TBD will be resolved in this report's §1 + §5 verification matrix rows 15-16 + §6 PR title + §6 manual-dispatch fallback + §6 CodeRabbit review (6 placeholders to resolve) + §9 row 6.8 squash-SHA cell in `Docs/AppFolio_Parity_Implementation_Plan_v2.md`.
