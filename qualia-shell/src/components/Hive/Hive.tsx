        ...
        <div style={{ display: 'flex', height: '100%', width: '100%', background: 'var(--bg-desktop)', color: 'var(--text-secondary)', fontFamily: 'inherit', fontSize: 13, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderBottom: '1px solid #222' }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-tertiary)', flex: 1 }}>The Hive · Agents</span>
                    <span style={{ fontSize: 11, color: '#666' }}>{openIds.size} running · {AGENTS.length} total</span>
                </div>

                {/* Cost attribution (§8.2) */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderBottom: '1px solid #161616', background: '#070707', fontSize: 11, color: 'var(--text-tertiary)' }}>
                    ...
                    <span style={{ color: 'var(--text-tertiary)' }}>· per-agent / per-domain attribution streams from backend observability when connected</span>
                </div>
                    ...
                    {AGENTS.map((a) => {
                        ...
                        return (
                            <div key={a.id} className="spotlight-card" style={{ borderRadius: 10, padding: 12, background: '#0c0c0c', display: 'flex', flexDirection: 'column', gap: 8 }}>
                                ...
                                <div style={{ fontSize: 11, color: 'var(--text-tertiary)', minHeight: 28 }}>{a.blurb}</div>
            ...
            <div style={{ width: 280, flexShrink: 0, borderLeft: '1px solid #222', background: '#070707', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 12px', borderBottom: '1px solid #222' }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-tertiary)', flex: 1 }}>CoPaw memory ({memory.length})</span>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: 8 }}>
                    {memory.length === 0 ? (
                        <div style={{ padding: 12, color: 'var(--text-tertiary)', fontSize: 11, lineHeight: 1.6 }}>No facts yet. Run an agent (Synthesis, Builder Agents) and key facts are captured here automatically.</div>
                    ) : memory.map((f) => (
                        <div key={f.id} style={{ padding: '7px 9px', marginBottom: 5, border: '1px solid #1c1c1c', borderRadius: 6, background: 'var(--bg-desktop)' }}>
                ...
                <button onClick={() => openWindow('honcho', 'Honcho', '')}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, margin: 10, padding: '8px 0', borderRadius: 7, border: '1px solid #333', background: 'transparent', color: 'var(--text-secondary)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
