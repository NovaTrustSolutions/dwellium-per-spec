        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', background: 'var(--bg-desktop)', color: 'var(--text-secondary)', fontFamily: 'inherit', fontSize: 13, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '10px 16px', borderBottom: '1px solid #222', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-tertiary)', flex: 1 }}>Foundry · Intake</span>
                    {items.length > 0 && <button onClick={() => clearFoundry()} title="Clear all items" style={{ background: 'none', border: '1px solid #333', color: 'var(--text-tertiary)', borderRadius: 5, padding: '3px 9px', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Clear</button>}
                </div>
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                    {PIPE.map((p, i) => (
                        <span key={p} style={{ fontSize: 10, padding: '2px 9px', borderRadius: 999, background: 'color-mix(in srgb, var(--accent) 6%, transparent)', border: '1px solid #222', color: 'var(--text-tertiary)', letterSpacing: '0.04em' }}>{i + 1}. {p}</span>
                    ))}
                </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
                ...
                <div style={{ border: '1px solid #222', borderRadius: 8, padding: 12, marginBottom: 18, background: '#070707' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                        ...
                        <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="Source URL (optional)" style={{ flex: 1, background: 'var(--bg-desktop)', border: '1px solid #333', borderRadius: 6, color: 'var(--text-primary)', fontSize: 12, padding: '6px 9px', outline: 'none', fontFamily: 'inherit' }} />
                    </div>
                    <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Paste content to capture into the knowledge pipeline…" rows={4} style={{ width: '100%', boxSizing: 'border-box', background: 'var(--bg-desktop)', border: '1px solid #333', borderRadius: 6, color: 'var(--text-primary)', fontSize: 13, padding: '8px 10px', outline: 'none', fontFamily: 'inherit', resize: 'vertical' }} />
...
function Empty({ text }: { text: string }) {
    return <div style={{ padding: '12px', color: 'var(--text-tertiary)', fontSize: 12 }}>{text}</div>;
}
...
function ReviewCard({ item, onAdmit, onReject, onTags, onTarget }: {
    ...
    return (
        <div style={{ border: '1px solid #222', borderRadius: 8, padding: 12, marginBottom: 10, background: 'var(--bg-desktop)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                ...
                {item.triagedBy && <span style={{ fontSize: 9, color: 'var(--text-tertiary)' }}>· {item.triagedBy}</span>}
            </div>
            ...
            {item.assessment && <div style={{ fontSize: 11, color: 'var(--text-tertiary)', fontStyle: 'italic', marginBottom: 8 }}>{item.assessment}</div>}
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <input value={tagText} onChange={(e) => { setTagText(e.target.value); onTags(e.target.value.split(',').map((s) => s.trim()).filter(Boolean)); }} placeholder="tags, comma, separated" style={{ flex: 1, background: 'var(--bg-desktop)', border: '1px solid #2a2a2a', borderRadius: 5, color: '#ddd', fontSize: 11, padding: '5px 8px', outline: 'none', fontFamily: 'inherit' }} />
                <input value={item.target ?? ''} onChange={(e) => onTarget(e.target.value || null)} placeholder="target location" style={{ flex: 1, background: 'var(--bg-desktop)', border: '1px solid #2a2a2a', borderRadius: 5, color: '#ddd', fontSize: 11, padding: '5px 8px', outline: 'none', fontFamily: 'inherit' }} />
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={onAdmit} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 6, border: 'none', background: ACCENT, color: 'var(--text-inverse)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}><Check size={13} /> Admit</button>
...
function MiniRow({ item, kind }: { item: FoundryItem; kind: 'admitted' | 'rejected' }) {
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', marginBottom: 4, border: '1px solid #1a1a1a', borderRadius: 6, background: '#070707' }}>
            ...
            {item.tags.length > 0 && <span style={{ fontSize: 9, color: 'var(--text-tertiary)' }}>{item.tags.slice(0, 3).join(' · ')}</span>}
        </div>
    );
}
