...
PROMPT_FILE="${PROMPT_FILE:-$PROJECT_DIR/Scripts/autorun/CLEANUP_UI_AUTORUN_PROMPT.md}"
LOG_DIR="${LOG_DIR:-$PROJECT_DIR/Scripts/autorun/logs}"
STATUS_FILE="$LOG_DIR/CLEANUP_AUTORUN_STATUS.md"
...
ts(){ date '+%Y-%m-%d %H:%M:%S'; }
log(){ echo "[$(ts)] $*" | tee -a "$LOG_DIR/cleanup_runner.log"; }
write_status(){
  { echo "# Cleanup+UI autorun status"; echo
...
log "=== Cleanup+UI autorun starting (MAX_HOURS=$MAX_HOURS) ==="
...
while true; do
  ...
  ITER=$((ITER+1))
  ITER_LOG="$LOG_DIR/cleanup_iter_$(printf '%03d' "$ITER")_$(date +%H%M%S).log"
  ...
  if [[ -n "$(git -C "$PROJECT_DIR" status --porcelain --untracked-files=no)" ]]; then
    git -C "$PROJECT_DIR" commit -q -am "autorun(cleanup safety-net): iteration $ITER ($(ts))" \
...
echo "         cat \"$PROJECT_DIR/Scripts/autorun/FEATURE_STATUS.md\""
echo "         cat \"$PROJECT_DIR/Scripts/autorun/BROKEN_LEDGER.md\""
echo "         open \"$PROJECT_DIR/Scripts/autorun/cleanup-shots/\""
