...
**Phase status.** ✓ FULL CLOSURE 2026-05-16 at Phase-7 closer (Phase-7 100% complete — 14 of 14 tasks closed + closer; **Phase-7 closer SHIPPED at squash SHA `TBD` (PR #TBD; resolution at Phase-8+ opener sweep per 25-consecutive-cross-phase-sweep-resolutions convention extending 24-pattern at 7.13 → 25-pattern at closer → 26-pattern at Phase-8+ opener); closer = sequential closer FINAL Phase-7 task** — sister to Phase-1/3/4/5/6 single-closure-per-phase precedents). Closure narrative at `Docs/Phase7_Closure_Report.md` (323 lines / 74,321 bytes; 8-section template byte-shape-mirror of `Docs/Phase6_Closure_Report.md` 272 lines / 51,782 bytes; +41% byte-count empirically-justified per v2.60.6 §4g content-density-driven scope codification). **🎯 Substantive Phase-7 engineering-finding catalog cemented at 2 publishable-level findings:** (A) Lazy-load IS structurally-correct lever family at React 19 + Vite 6 + 4,500 ms-LCP baseline architecture; (B) `vi.useFakeTimers` + React 19 scheduler primitives is structurally fragile when component-under-test has async-cascade-to-render path. **🎯 4 NEW Phase-7-introduced classes** (CI-CONFIG-ONLY 12th + BASELINE-ARTIFACT 13th + PERF-LEVER-LAZY-LOAD 14th + TEST-INFRA-FIX 15th; retroactive correction at closer per Option A locked — original 7.13-close-time framing was anchor-bias artifact citing "2 NEW classes"; empirically-correct count cemented at closer §3). **🎯 6 GR-15 PERMANENT process change candidates** v2.60.1-v2.60.6 + **3-pattern anchor-bias-mitigation finding cluster**. **🎯 25 consecutive cross-phase sweep-resolutions cemented at Phase-7 closer sweep** (7.13 TBD → `0db9f2b` / `#67` co-shipped). Phase-8+ becomes next-phase boundary; Phase-8+ Block B opener recommended: SSR-rendered shell exploration per Finding (A) + v1 L228 ≤500 ms LCP STRUCTURALLY UNATTAINABLE empirically refined to "multi-lever within React 19 + Vite 6 architecture". R OPEN 2026-05-11; **Task 7.1 OPENER closed at squash SHA `ddf1404` (PR #56) 2026-05-11**; **Task 7.2 closed at squash SHA `9126cc2` (PR #57) 2026-05-11**; **Task 7.3 closed at squash SHA `a8f1a10` (PR #58) 2026-05-12**. **Task 7.4 closed at squash SHA `cd26ce4` (PR #59) 2026-05-13**. **Task 7.5 closed at squash SHA `16c2ac2` (PR #60) 2026-05-13** (Cowork Option A manual PR-open salvage; Block A item #7 opportunistic co-closure). **Task 7.6 closed at squash SHA `7b771ec` (PR #61) 2026-05-13**. **Task 7.8 closed at squash SHA `f0b0127` (PR #62) 2026-05-13** (DOC-only-empirical-void-closure per Cowork Option E — v2.51.1 timeout-bump at 7.4 absorbed compliance-row flake as side-effect; Block A 8-of-8 CLOSED). **Task 7.9 closed at squash SHA `b89d727` (PR #63) 2026-05-13** (MEASUREMENT-ONLY-with-empirical-finding-and-revert per Cowork OUTCOME B — Lever 2 vendor split 2-round attempt + clean revert). **Task 7.10 closed at squash SHA `6a7eab5` (PR #64) 2026-05-15** (OUTCOME C ship-lazy-load-alone partial-win — Lever 3 React.lazy expansion of App.tsx eager imports ships with empirical LCP n=3 mean −550 ms / −12.4%). **Task 7.11 closed at squash SHA `7ff4d5e` (PR #65) 2026-05-15** (MEASUREMENT-ONLY — n=10 Lighthouse variance characterization at HEAD-post-7.10; LCP CV 2.29% first non-degenerate noise-floor metric). **Task 7.14 closed at squash SHA `e657b89` (PR #66) 2026-05-15** (DOC-only-historical-footnote-correction per Cowork PRE0 Q1-Q3 verdicts — inline empirical-correction footnote at `Docs/Phase5_Perf_Report.md §2` preserving Phase-0-era stale-claim verbatim + adding Phase-7-era empirical correction trajectory across 6.7 + 7.10 + 7.11 closes; NEW MEASUREMENT-ONLY sub-shape `DOC-only-historical-footnote-correction` docked; class 8pt → 9pt cross-phase; resolved at 7.13 sweep per 24-consecutive-cross-phase-sweep-resolutions convention extending 23-pattern at 7.14 → 24-pattern at 7.13). **Task 7.13 closed 2026-05-15 at squash SHA `0db9f2b` (PR #67; resolved at Phase-7 closer sweep per 25-consecutive-cross-phase-sweep-resolutions convention; TEST-INFRA-FIX closure shape — eliminated `vi.useFakeTimers()` from sister grid-regression + upcoming-events list RTL tests at `qualia-shell/src/test/appfolioParity/calendar.test.tsx`; root cause empirically isolated as React 19 scheduler primitive interaction with fake-timer-mocked `MessageChannel`/microtask queue stranding pending render commits across `useFakeTimers → useRealTimers` handoff; replaced with `vi.setSystemTime()` alone + `waitFor()` polling on real clock; pre-edit 20/20 FAIL → post-edit 20/20 PASS binary deterministic variance-collapse; falsified-hypothesis trajectory documented; NEW class TEST-INFRA-FIX docked at project-wide 15th cumulative; full vitest 259/259 LOCAL clean — FIRST since Phase-6 6.3).** **🎯 Block A 8-of-8 CLOSED at 7.8.** **🎯 Block B 3-of-3 FULLY CLOSED milestone at 7.11** (7.9 empirical-void + 7.10 OUTCOME C SHIP + 7.11 MEASUREMENT-ONLY). **🎯 Block C 3-of-3 FULLY CLOSED milestone at 7.13** (7.12 ✓ co-closed at 7.3 v2.50.1+v2.50.2 + 7.14 ✓ at e657b89 + **7.13 ✓ at this commit**). **🎯 Phase-7 14-of-14 ✓ FULL CLOSURE milestone at 7.13** (sister-shape to Phase-6 11-of-11 at 6.9 close; SECOND PHASE in project history with substantive engineering-finding catalog cemented at FULL CLOSURE). **🎯 Substantive Phase-7 engineering-finding catalog cemented at 2 publishable-level findings:** **(A) Lazy-load IS structurally-correct lever family at React 19 + Vite 6 + 4,500 ms-LCP baseline architecture** (cemented across 4 cross-referenced closes 6.7 + 7.10 + 7.11 + 7.14); **(B) `vi.useFakeTimers` + React 19 scheduler primitives is structurally fragile when component-under-test has async-cascade-to-render path** (cemented at 7.13). **🎯 NEW classes Phase-7-introduced: PERF-LEVER-LAZY-LOAD (14th project-wide; 7.10) + TEST-INFRA-FIX (15th project-wide; 7.13) = 2 NEW Phase-7-introduced classes** (sister to Phase-6's COMPONENT-FIX + CLOSURE-NARRATIVE-CONSOLIDATION = 2 NEW Phase-6-introduced classes). **🎯 3pt cross-phase deterministic-validation pattern empirically calibrated at 7.13** (7.10 perf-lever variance collapse + 7.11 n=10 noise-floor + 7.13 test-infra-fix binary inversion). **Block A** (a11y + CI architecture; 8 items 7.1-7.8): ALL ✓ / **Block B** (perf multi-lever arc; 3 items 7.9-7.11): ALL ✓ at 7.11 / **Block C** (test infrastructure stabilization; 3 items 7.12-7.14): ALL ✓ at 7.13. **Phase-7 progress 14 of 14 ✓; FULL CLOSURE achieved.** **🎯 Phase-7 §9 main matrix column ADDED at OPENING per Process Improvement #1 from `Docs/Phase6_Closure_Report.md §8`**.
...
#### Task 7.8 — Linux CI render-timing failure investigation on `compliance-row-workersCompExpiration` ✓ CLOSED 2026-05-13 (DOC-only-empirical-void-closure)

**Goal.** Investigate the Linux CI render-timing failure on `compliance-row-workersCompExpiration` testid (post-6.8 PR-open finding; structurally parallel to Block A items 7.4-7.6 but distinct failure mode — render-timing race not pixel-diff).

**Empirical execution.** PRE0 6-question gate empirically located the origin failure at PR #54 (6.8) parity gate run [25659376515](https://github.com/NovaTrustSolutions/dwellium-per-spec/actions/runs/25659376515) — the run was SUCCESS-overall because the conjoined Playwright baseline E2E step had `continue-on-error: true` at that time. Failure mode: `expect(row).toBeVisible()` waits-for-locator timeout at `qualia-shell/e2e/appfolio-parity-vendor-compliance.spec.ts:148-149` (`workersCompExpiration` is the 1st of 5 `EMPTY_KEYS` so it's the first locator to flake when render-timing races). **🎯 The v2.51.1 timeout-bump (60_000 → 90_000 ms on `playwright.baseline.config.ts:69` at Task 7.4) absorbed this flake as a side-effect of the Vendors axe-scan absorption.** 4 consecutive 7.x parity-gate runs (25775177388 + 25780671522 + 25787465352 + 25789452205) PASS the compliance-row test under the 90s budget, including the 7.6 FIRST-TIME-BLOCKING parity-gate run 25787465352 (screenshot-baseline step 16 passed in 4.0m under `continue-on-error: false` with feature spec compliance-row test inside). Darwin local Q3 repro: ✓ PASS in 6.3s. Cowork Option E verdict (Empirical NO-OP / DOC-only-empirical-void-closure); no spec edit needed.

**Files touched.** 0 source/spec/config files (DOC-only closure). 5 doc files: NEW `Docs/Phase7_Task_7_8_Completion_Report.md` + UPDATE `Docs/AppFolio_Parity_Implementation_Plan_v2.md` v2.54 amendment + UPDATE `Docs/Phases/Phase_7_Plan.md` row 7.8 closure + UPDATE `Docs/Phase7_Task_7_6_Completion_Report.md` 7.6 TBD → `7b771ec` / `#61` resolution + UPDATE `CLAUDE.md` HEAD pointer pivot.

**Calibration class.** **MEASUREMENT-ONLY carry-over (extends 5pt → 6pt cross-phase) with NEW sub-shape "DOC-only-empirical-void-closure"** (project-wide class count stays at 13; 5 sub-shapes calibrated under MEASUREMENT-ONLY). Sister-shape constellation: opportunistic-closure pattern now 3-of-3 within Phase-7 (Block A item #7 ← 7.5 + Block C item #1 ← 7.3 v2.50.1+v2.50.2 + Block A item #8 ← 7.4 v2.51.1). Reclassified on-the-fly per kickoff brief authorization — Phase_7_Plan row 7.8 originally projected E2E-PLAYWRIGHT carry-over assuming a per-test timeout spec-edit shape; empirical execution is DOC-only so MEASUREMENT-ONLY applies.

**🎯 NEW Phase-7 deferred-item #3:** GR-15 amendment candidate for Plan v2.55+: "Timeout-bump empirical reach may exceed original scope" PRE-FLIGHT discipline note (sister to 7.5's first-real-execution-as-truth-signal; together form 2-of-2 cross-phase calibration of "empirical-reach vs amendment-intent" PRE-FLIGHT discipline class).
...
#### Task 7.9 — Lever 2 `manualChunks` vendor split via `vite.config.ts` ✓ CLOSED 2026-05-13 (MEASUREMENT-ONLY-with-empirical-finding-and-revert; sister to 6.7 Lever 1 REVERT)

**Goal (attempted).** Configure `vite.config.ts::build.rollupOptions.output.manualChunks` to split vendor dependencies into separate chunks targeting LCP reduction via parallel vendor-chunk loading.

**Empirical execution (2-round attempt + REVERT).** Round-1 conservative shape `'react-vendor': ['react', 'react-dom']` produced NO-OP LCP signal (n=3 mean delta +1.4 ms vs pre-edit baseline 4,602.3 ms; vendor chunk only 11,794 B because React 19's runtime weight lives in `react-dom/client` + `scheduler` packages NOT matched by bare entry IDs). Round-2 v2.55.1 in-place scope expansion to `['react','react-dom','react-dom/client','scheduler']` + `'icons-vendor': ['lucide-react']` achieved STRUCTURAL chunk-axis BREAK SUCCESS (index eager-chunk −217,927 B / −36.5%; +2 new vendor chunks aggregating 264,304 B) but empirical LCP signal STILL NO-OP (n=3 mean delta −24.6 ms / −0.5%; median UNCHANGED at 4,504 ms; gate ≥200 ms NOT MET). Cowork OUTCOME B verdict per kickoff brief Step-B+4: clean revert via `git checkout HEAD -- qualia-shell/vite.config.ts`. Sister-shape to 6.7 Lever 1 font deferral REVERT; **2pt cross-phase calibration of perf-lever underperformance pattern empirically established at 7.9 close**.

**Files touched at 7.9 close.** 0 source/spec/config files (vite.config.ts reverted to HEAD-post-7.6 canonical). 5 doc files: NEW `Docs/Phase7_Task_7_9_Completion_Report.md` + UPDATE Plan v2.55 amendment + UPDATE this Phase_7_Plan row 7.9 + UPDATE row 7.10 scope-expansion note + UPDATE `Docs/Phase7_Task_7_8_Completion_Report.md` 7.8 TBD → `f0b0127` resolution + UPDATE `CLAUDE.md` HEAD pointer pivot.

**Calibration class.** **MEASUREMENT-ONLY carry-over (extends 6pt → 7pt cross-phase) with NEW sub-shape "WITH-EMPIRICAL-FINDING-AND-REVERT-PERF-LEVER"** (project-wide class count stays at 13; 6 sub-shapes calibrated). Sister-shape to 6.7 Lever 1 REVERT shape; 2pt cross-phase calibration of perf-lever underperformance pattern.

**🎯 Substantive Phase-7 engineering finding for Phase-7 Closure Report:** **Piecemeal Lever-by-Lever perf optimization is structurally insufficient at React 19 + Vite 6 + 4,500 ms-LCP baseline architecture.** Probable root cause: LCP bottleneck is JS execution + parse + render time on initial paint, NOT critical-path bytes-downloaded. Vendor extraction (even 218 KB / −36.5%) doesn't move the needle because parallel HTTP/2 streams save marginal time when browser wasn't bottlenecked on serial download.

**🎯 NEW Phase-7 deferred-item #4** (GR-15 amendment candidate for Plan v2.56+): "Perf-lever piecemeal pattern empirically void at React 19 + Vite + 4,500 ms-LCP baseline; future perf work should be lazy-load-first + vendor-split-only-if-empirically-helpful." Sister to deferred-items #1-3; forms 4-of-4 cross-phase "empirical-reality vs amendment-intent" PRE-FLIGHT discipline constellation.

**v2.55.1 in-place addendum (2026-05-14) — defensive workflow YAML patch for GitHub Actions artifact storage quota exhaustion per Cowork Option C verdict.** PR #63 parity gate run [25829682491](https://github.com/NovaTrustSolutions/dwellium-per-spec/actions/runs/25829682491) (2026-05-13) reported job conclusion=`failure` from ONLY step-13 "Upload baseline artifacts" hitting `Failed to CreateArtifact: Artifact storage quota has been hit` despite all 12 substantive steps PASSING. v2.55.1 adds `continue-on-error: true` to `.github/workflows/appfolio-parity-gate.yml::Upload baseline artifacts` step + ~14-line comment block. Sister-shape to 7.4 v2.51.1 + 7.3 v2.50.1/v2.50.2 in-place CI-flake-tolerance-policy patches. CI-CONFIG-ONLY class workflow-step continue-on-error sub-domain 3rd data point (7.3 false + 7.6 false + 7.9 v2.55.1 TRUE; bidirectional-calibrated). **Reframes deferred-item #5**: original "quota exhaustion blocks merge" framing resolved by patch; remaining carry-forward = Plan v2.56+ amendment candidate to lower `retention-days: 14` → 7 or less for long-term quota hygiene.

**🎯 Block B strategic pivot at 7.9 close:** 7.10 absorbs Lever 2 + Lever 3 combined scope (see row 7.10 below). Block B 3-item arc effectively becomes 2-task arc.

**(Original Goal preserved for historical reference):** Configure `vite.config.ts::build.rollupOptions.output.manualChunks` to split vendor dependencies into separate chunks (currently `vite.config.ts` has NO `manualChunks` configured per Phase-6 6.7 PRE0 Q4 empirical confirmation — all vendor code is baked into the monolithic `index-CTl84rdZ.js` 597 KB chunk).
...
#### Task 7.10 — Lever 3 lazy-load App.tsx eager imports (SCOPE EXPANDED at 7.9 close to absorb Lever 2 combined scope per Cowork strategic pivot)

**Scope expansion at 7.9 close.** Per Cowork OUTCOME B verdict at 7.9 close + sister-shape 2pt cross-phase calibration of perf-lever underperformance pattern (6.7 Lever 1 + 7.9 Lever 2 both REVERT): **7.10 absorbs Lever 2 + Lever 3 combined scope.** Lazy-load App.tsx eagerly-imported components is the primary lever; vendor extraction (Lever 2 from 7.9) becomes an optional secondary lever only if empirically helpful when stacked with lazy-load. The substantive Phase-7 engineering finding from 7.9 (piecemeal Lever-by-Lever perf optimization structurally insufficient at React 19 + Vite 6 + 4,500 ms-LCP baseline architecture) informs the expanded 7.10 strategy: stack levers in a single substantive task with empirical-reality-first PRE-FLIGHT discipline.

**Goal.** Convert App.tsx eager imports (TenantPortal / SecurityPortal / OpenJarvisWidget / etc. — exact inventory at Phase-7 PRE0 of 7.10) to `React.lazy()` dynamic imports so they're not bundled into the initial-paint `index-*.js` chunk. Optionally combine with vendor extraction (manualChunks) ONLY if empirical signal post-lazy-load shows insufficient LCP improvement AND a specific direct dep is identified as initial-paint-blocking heavy.

**Files touched.** 1 source file: `qualia-shell/src/App.tsx` + possibly `Suspense` boundary additions. Optionally `qualia-shell/vite.config.ts` if vendor extraction is empirically helpful as secondary lever.

**Acceptance gate.** Post-edit Lighthouse measurement (n=10+ per 7.11) shows LCP delta vs post-7.9 state (= same as HEAD-post-7.6 since 7.9 REVERT restored byte-for-byte). PRIMARY ≤3,000 ms target retained as Block B exit gate. **Empirical-reality fallback per 7.9 sister precedent:** if lazy-load alone produces NO-OP LCP signal, Cowork verdict on REVERT vs vendor-extraction-secondary-lever-stacked vs Phase-8+ SSR pivot is the branch point (analogous to 7.9 Step-B+4 outcome-branching structure).

**Calibration class.** **PRODUCTION-SOURCE-EDIT** (chunk-axis BREAK expected; 5th cross-phase production-source-edit data point after 6.1a + 6.3 + 6.4 + 7.1) with sub-shape candidate **PERF-LEVER-LAZY-LOAD** (1st data point; sister-shape constellation to PRODUCTION-SOURCE-EDIT-VENDOR-SPLIT-REVERT class shape from 7.9 retroactively).
