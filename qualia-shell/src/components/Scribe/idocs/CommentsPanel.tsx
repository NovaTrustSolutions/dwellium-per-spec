...
function Thread({ c, docId, cardId, author, blockName }: { c: BlockComment; docId: string; cardId: string; author: string; blockName?: string }) {
    ...
    return (
        <li className={`scribe-idocs-ed__thread${c.resolved ? ' is-resolved' : ''}`} data-testid={`idoc-comment-${c.id}`}>
            ...
            <div className="scribe-idocs-ed__thread-actions">
...
export default function CommentsPanel({ docId, card, author, blockId, onScope, onClose }: CommentsPanelProps) {
    ...
    return (
        <aside className="scribe-idocs-ed__drawer scribe-idocs-ed__comments" aria-label="Comments" data-testid="idoc-comments">
            ...
            <form className="scribe-idocs-ed__composer" onSubmit={(e) => { e.preventDefault(); submit(); }}>
                ...
                <button type="submit" className="scribe-idocs__btn scribe-idocs__btn--primary" disabled={!text.trim()}>Comment</button>
            </form>
            ...
            <ul className="scribe-idocs-ed__threads">
                {visible.map((c) => { const b = blockOf(c.blockId); return <Thread key={c.id} c={c} docId={docId} cardId={card.id} author={author} blockName={!blockId && b ? blockLabel(b) : undefined} />; })}
            </ul>
        </aside>
    );
}
