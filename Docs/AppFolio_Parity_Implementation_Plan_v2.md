...
Pending row: `3.1, 3.2, 3.3, 3.4, 3.9` (Task 3.7 closed 2026-04-25 at squash SHA `fe9b642`; Task 3.8 closed 2026-04-25 at squash SHA `b4b7c9a` — same-day double-close on the retrofit chain). Phase-3 column flips `R` → `✓` when the last row closes. Phase-3 GR-13 retrofit chain (3.7 → 3.8 → 3.9) is sequential by design (each rebases on prior to inherit the Inner-wrapper + ErrorBoundary precedent without churn); 3.1–3.4 are the parallel batch from §19 dependency graph and remain unblocked by the retrofit chain. Task 3.7 was the first Phase-3 task to land (closed Task 2.8 §7 "ProjectsModule" entry + Task 2.9 (f) §7 + §8 row 8 "ProjectsModule GR-13 retrofit" — see Changelog v2.11). Task 3.8 is the second to land (closed Task 2.8 §7 "CorporateReview" entry + Task 3.7 §7 entry #3 Playwright workaround — VALIDATED at Task 3.8 CDP probe; see Changelog v2.12). Phase-3 deferred-items ledger drops 8 → 6 with Task 3.8's close. Appendix D Phase-3 column was UNTOUCHED through Task 3.7 (precedent extended PRs #8 → #19); Task 3.8 supersedes (4 row updates + 1 NEW row for `corporate_review.json`) — see Changelog v2.12.
