# AppFolio Parity Implementation Plan — v2.6

**Version.** 2.6 (2026-04-24). Incremental clarifications after v2.5 — documents Task 2.10 (PropertyTimeline multi-source merge for 128 Buena Vista) close as the third Phase-2 general-pool task post-B3. Changes tracked in the Changelog at the bottom.
...
| 2.10 — PropertyTimeline: multi-source merge for 128 Buena Vista | ✓ | (populated by this PR's squash-to-main) | 2026-04-24 | `Docs/Phase2_Task_2_10_Completion_Report.md` (same 8-section template as Task 2.3 / 2.5 / 2.7 / 2.2 / 2.1; backs every ✓ cell with a section reference) |
| 2.4, 2.6, 2.8, 2.9 | pending | — | — | — |

Task-2.3 / 2.5 / 2.7 closures reference Appendix D row 1's pre-recorded Phase-2 ownership of `packages/types/index.ts` (serial 2.3 → 2.5 → 2.7; text landed in PR #8 / `1bb7518` and preserved by all three PRs without re-edit). Task 2.5 rebased onto Task 2.3's type additions; Task 2.7 rebased onto Task 2.5's. **B3 serial chain CLOSED** at Task 2.7 merge (HEAD `40875db`): `packages/types/index.ts` Phase-2 serial ownership retires, and the remaining Phase-2 tasks open to the general pool per Appendix D's illustrative treatment. Task 2.2 (Communication seed, `b98e84c`) was the first general-pool task landed post-B3; Task 2.1 (Calendar AHA inspection seed, `67768c9`) was the second; Task 2.10 (PropertyTimeline multi-source merge, this PR) is the third. Task 2.10 is READ-ONLY on `properties.json` — the Appendix D 2.4 + 2.10 sequential constraint is non-conflict for this PR (Task 2.4's forecast 50-property seed remains pending and will write to properties.json first when landed).
