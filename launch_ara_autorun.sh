#!/usr/bin/env bash
###############################################################################
# launch_ara_autorun.sh — 2-HOUR UNATTENDED Claude Code loop
#                         (ARA / Stella / Inbox Zero + linkage arc)
#
...
#   chmod +x launch_ara_autorun.sh
#   ./launch_ara_autorun.sh
#   MAX_HOURS=2 ./launch_ara_autorun.sh
#   ./launch_ara_autorun.sh --print      # write prompt + show cmd, run nothing
#
...
PROMPT_FILE="${PROMPT_FILE:-$PROJECT_DIR/Scripts/autorun/ARA_STELLA_INBOX_AUTORUN_PROMPT.md}"
LOG_DIR="${LOG_DIR:-$PROJECT_DIR/Scripts/autorun/logs}"
STATUS_FILE="$LOG_DIR/ARA_AUTORUN_STATUS.md"
...
# ----------------------------- HELPERS --------------------------------------
ts()  { date '+%Y-%m-%d %H:%M:%S'; }
log() { echo "[$(ts)] $*" | tee -a "$LOG_DIR/ara_autorun_runner.log"; }

write_status() {
  {
    echo "# ARA/Stella/Inbox autorun status"
    echo
    echo "- **State:** ${1:-?}"
    echo "- **Note:** ${2:-}"
...
# ----------------------------- PREFLIGHT ------------------------------------
log "=== ARA/Stella/Inbox autorun starting (MAX_HOURS=$MAX_HOURS, MAX_ITERS=$MAX_ITERS) ==="
...
log "Current branch: $CUR_BRANCH (the prompt creates/checks out feat/ara-stella-inbox-linkage on iteration 1)."
...
while true; do
  ...
  ITER=$((ITER+1))
  ITER_LOG="$LOG_DIR/ara_iter_$(printf '%03d' "$ITER")_$(date +%H%M%S).log"
...
echo "         cat \"$PROJECT_DIR/Scripts/autorun/ARA_PROGRESS.md\""
echo "         cat \"$PROJECT_DIR/Scripts/autorun/LINKAGE.md\""
echo "Push when satisfied (main first):"
echo "  git -C \"$PROJECT_DIR\" push origin main"
echo "  git -C \"$PROJECT_DIR\" push -u origin feat/ara-stella-inbox-linkage"
