    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', background: 'var(--bg-desktop)', color: 'var(--text-secondary)', fontFamily: 'inherit', fontSize: 13, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderBottom: '1px solid #222', flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-tertiary)', flex: 1 }}>Autonomous Runs</span>
                <button onClick={openTerminal} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 6, border: '1px solid #333', background: 'transparent', color: 'var(--text-secondary)', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                    <TerminalSquare size={13} /> Open Terminal
                </button>
            </div>
            <div style={{ padding: '8px 16px', borderBottom: '1px solid #161616', fontSize: 11, color: 'var(--text-tertiary)' }}>
                ...
                {RUNS.map((r) => (
                    <div key={r.id} className="spotlight-card" style={{ borderRadius: 10, padding: 12, background: '#0c0c0c' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                            <span style={{ fontWeight: 700, color: 'var(--text-primary)', flex: 1, fontSize: 13 }}>{r.name}</span>
                            <span style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 10, color: 'var(--text-tertiary)' }}><Clock size={11} /> {r.eta}</span>
                        </div>
                        <div style={{ fontSize: 11.5, color: 'var(--text-tertiary)', marginBottom: 10, lineHeight: 1.5 }}>{r.blurb}</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <code style={{ flex: 1, fontSize: 11, color: ACCENT, background: 'var(--bg-desktop)', border: '1px solid #222', borderRadius: 5, padding: '5px 8px', fontFamily: "'JetBrains Mono','Fira Code',monospace", overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.cmd}</code>
