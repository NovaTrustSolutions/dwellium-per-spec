/**
 * HalocronWorkspaces — user-created, persistent workspaces inside Halocron OS.
 *
...
export default function HalocronWorkspaces() {
    ...
    const createWorkspace = () => {
        ...
        if (!name) return;
        const ws: Workspace = { id: newWorkspaceId(), name, appIds: [], split: 2, layout: 'grid', frames: {}, notes: '', updatedAt: Date.now() };
    ...
    const removeApp = (id: string) => { if (open) patch(open.id, { appIds: open.appIds.filter((a) => a !== id) }); };
    ...
    const startDrag = (e: React.PointerEvent, id: string, i: number, mode: 'move' | 'resize') => {
        ...
        <div className="ws-run">
            <div className="ws-run__bar">
                ...
                <div className="ws-split">
                    ...
                    {[1, 2, 3].map((n) => (
                        <button key={n} className={open.layout === 'grid' && open.split === n ? 'on' : ''} onClick={() => patch(open.id, { layout: 'grid', split: n as 1 | 2 | 3 })}>{n}</button>
                    ))}
                    ...
                    {open.appIds.map((id, i) => {
                        ...
                        return (
                            <div key={id} className="ws-pane ws-pane--float" style={{ left: `${f.x}%`, top: `${f.y}%`, width: `${f.w}%`, height: `${f.h}%` }}>
                                ...
                                <div className="ws-resize" onPointerDown={(e) => startDrag(e, id, i, 'resize')} title="Drag to resize" />
                            </div>
                        );
                    })}
                </div>
