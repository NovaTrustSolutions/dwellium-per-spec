...
> **¹ Task 8.4 Finding J / D-1 LOCK footnote-correction (shipped 2026-05-17):** ThemeProvider's empirical signature at HEAD-post-Task-8.4 is **6 init-time `localStorage.getItem()` reads** (4 primary keys: `dwellium-theme` + `dwellium-font-pairing` + `dwellium-accent-color` + `dwellium-animations` + 2 legacy fallbacks: `qualia-theme` + `qualia-accent-color`), not 8. The "8 init-time reads" framing above was hypothetical pre-empirical-verification. Structural SSR-safety verdict UNCHANGED — ThemeProvider remains STRUCTURALLY UNSAFE for SSR; only the precise count + key-name inventory was off. See §3.1 below for canonical post-correction inventory.
>
> **² Task 8.9 Finding Z / Verdict 5(a)+(b) LOCK footnote-correction (shipped 2026-05-18):** UserProvider's empirical signature at HEAD-post-Task-8.8 (re-verified at Task 8.9 PRE0) is `useState<string | null>(() => localStorage.getItem(TOKEN_KEY))` at `UserContext.tsx:52` where `TOKEN_KEY = 'dwellium-auth-token'` (L14 const) — **for TOKEN restoration, not USER restoration**. The audit doc's §3.2 below cites this correctly via the code excerpt at L228; however, **the `CLAUDE.md` Conventions block per-provider-SSR-safety taxonomy entry** cited a hypothetical `useState(() => localStorage.getItem('qualia_auth_user'))` framing (wrong key name + wrong subject) — the string `'qualia_auth_user'` does NOT exist anywhere in `qualia-shell/src/`. Actual user-data persistence key is `'dwellium-user'` (L237 setItem / L274 getItem), accessed inside async callbacks / useEffect bodies (effect-time SAFE per §2 taxonomy). Structural SSR-safety verdict UNCHANGED — UserProvider remained STRUCTURALLY UNSAFE due to the L52 token lazy init; only the empirical specifics in the cross-doc convention narrative differed. **Migrated to `useSyncExternalStore` at Task 8.9 close per PROVIDER-SSR-REMEDIATION class shape** (project-wide 19th cumulative class). CLAUDE.md Conventions entry refinement deferred to Phase-8+ closer per Cowork Verdict 5 trailing-clause LOCK (post-cleanup R5 state preservation).
...
> **🔗 Inline-footnote-correction at Phase-8+ Task 8.9 close (2026-05-18) per Cowork Verdict 5(a) LOCK + v2.64.0 audit-content empirical-vs-hypothetical PRE-FLIGHT discipline.** Empirical re-read at Task 8.9 Step-2 PRE0 confirmed this section's code excerpt at L228 is **empirically correct** — `useState<string | null>(() => localStorage.getItem(TOKEN_KEY))` where `TOKEN_KEY = 'dwellium-auth-token'`. The cross-doc drift surfaced at Task 8.9 PRE0 lived in `CLAUDE.md` Conventions block per-provider-SSR-safety taxonomy entry (which cited `localStorage.getItem('qualia_auth_user')` — hypothetical key that doesn't exist in source). **Audit-shipping-time content preserved verbatim below** (no in-section correction needed); this footnote cements Finding Z at the audit-doc altitude per sister-shape to §3.1's Task 8.4 D-1 LOCK ThemeProvider footnote byte-for-byte. **Migration deliverable:** L52 useState lazy initializer migrated to `useSyncExternalStore` + `tokenStore` (exported from `UserContext.tsx`) at Task 8.9 close per PROVIDER-SSR-REMEDIATION class (19th cumulative). `getServerSnapshot()` returns `null` (unauthenticated initial state); hydration flash for authenticated users at `ssr: true` altitudes captured as **Finding EE** for Task 8.11 architectural decision. **Cross-link:** `Docs/Phase8_Task_8_9_Completion_Report.md §0` Finding Z cementation.
