...
| 3.2 — (parallel batch; not yet started) | R | — | — | — |
| 3.3 — (parallel batch; not yet started) | R | — | — | — |
...
| 3.8 — CorporateReview: GR-13 retrofit — second PR in chain (rebases on 3.7 post-merge) | R | — | — | (pending) |
| 3.9 — TenantPortal: GR-13 retrofit — third PR in chain (rebases on 3.8 post-merge) | R | — | — | (pending) |

Pending row: `3.1, 3.2, 3.3, 3.4, 3.8, 3.9` (Task 3.7 closed 2026-04-25 at squash SHA `fe9b642`). Phase-3 column flips `R` → `✓` when the last row closes. Phase-3 GR-13 retrofit chain (3.7 → 3.8 → 3.9) is sequential by design (each rebases on prior to inherit the Inner-wrapper + ErrorBoundary precedent without churn); 3.1–3.4 are the parallel batch from §19 dependency graph and remain unblocked by the retrofit chain. Task 3.7 is the **first** Phase-3 task to land; it closes the consolidated Phase-3 deferred-items ledger entries from Task 2.8 §7 ("ProjectsModule" cross-module write-guard entry) and Task 2.9 (f) §7 + Task 2.9 §8 row 8 (full ProjectsModule GR-13 retrofit) — see Changelog v2.11. Appendix D Phase-3 column stays empty for all rows (Task 3.7 touches no fixture/handler/types files; only `ProjectsModule.tsx` + new test file + Docs).
...
|---|---|---|---|---|---|
| `packages/types/index.ts` | Task 1.1–1.5 sequentially | Phase-2 ownership: Task 2.3 → 2.5 → 2.7 (strictly serial). Tasks 2.5 and 2.7 rebase onto 2.3's type additions rather than modifying the file independently. | Task 3.8 (5th post-B3 additive amendment: `ReviewStatus` + `DocPriority` + `ReviewDocument` hoist from `CorporateReview.tsx:14-28`; precedent break from Task 3.7 which was the only post-B3 task to skip; v2.11 prediction "stays empty for all rows" superseded by v2.12) | — | Task 5.1a |
| `qualia-shell/src/components/StrataDashboard/strataTypes.ts` | shadow of ↑ | ↑ | ↑ (Task 3.8 adds 3 re-exports: `ReviewStatus`, `DocPriority`, `ReviewDocument`) | — | ↑ |
| `strataApi.static.ts` | Task 1.1 only | Task 2.* rebase onto each other | Task 3.8 (6 NEW handlers — 1 GET `/corporate-review` with status+search filter + 5 POST: upload [multipart with `static-upload-${randomUUID}` ID prefix] / triage / approve / reject / create-workitem; +1 export `strataUpload<T>` for FormData; +86 lines) | Task 4.7 only | — |
| `strataApi.backend.ts` | NO (GR-5) | NO | Task 3.8 (shape-contract addition only — `strataUpload<T>` re-export, +16 lines; required to keep static/backend impl-parity per `strataApi.ts:8-9` invariant; does NOT change real-backend logic, so GR-5 spirit preserved) | NO | Task 5.1b |
