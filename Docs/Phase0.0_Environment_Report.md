...
| 0.0.9 | Screenshot-diff baseline | ⚠ arm64-linux smoke only; dev-box run pending | Smoke-verified end-to-end flow: login → expand Property Management → open Strata → navigate to Overview → render fixture data (Occupied Units, Monthly Revenue, Occupancy chart, 87% collection). Playwright snapshot tooling produces platform-suffixed PNGs (`overview-chromium-linux.png`) — canonical baselines are `*-chromium-darwin.png` from the macOS dev box. |
...
4. **0.0.9 Screenshot baseline.** Sandbox smoke-proved the navigation flow works end-to-end with the updated spec. Dev box must run `npx playwright test --config playwright.baseline.config.ts --update-snapshots` to produce `overview-chromium-darwin.png` (etc.) — the Linux-arm64 PNGs the sandbox would write don't match macOS renderings.
...
- ⏳ 0.0.9 Screenshot baseline canonical (needs `--update-snapshots` for `*-chromium-darwin.png`)
- ⏳ Smoke CI PR through `.github/workflows/appfolio-parity-gate.yml` (requires one-line tweak to `node-version-file: '.nvmrc'` first)
